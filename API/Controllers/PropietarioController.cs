...
public class PropietarioController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
     public async Task<ActionResult<Pager<object>>> Consulta4APag([FromQuery] Params paisParams)
    {
        var entidad = await unitofwork.Propietarios.Consulta4A(paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
    ...
    {
        var entidad = await unitofwork.Propietarios.Consulta5B(paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
        var listEntidad = mapper.Map<List<object>>(entidad.registros);
        return new Pager<object>(listEntidad, entidad.totalRegistros, paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
    }
    [HttpPost]
